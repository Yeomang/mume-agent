...
**코드를 수정할 때마다 README.md도 반드시 함께 업데이트한다.**

### 업데이트 대상 판단 기준
다음 중 하나라도 해당하면 README를 수정한다:
- 새 파일 추가 또는 기존 파일 삭제/이름 변경
- 모듈의 역할이나 실행 흐름 변경
- API 엔드포인트 추가/삭제/변경
- 환경 변수 추가/삭제/변경
- 데이터 파이프라인 구조 변경
...
### 업데이트 방법
1. 변경된 내용에 해당하는 섹션을 수정한다.
2. 문서 상단의 `> **최종 업데이트:**` 날짜를 오늘 날짜로 갱신한다.
3. `## 변경 히스토리` 표에 새 행을 **맨 위**에 추가한다.
   - 형식: `| YYYY-MM-DD | 변경 내용 요약 (한 줄) |`
   - 날짜는 항상 `YYYY-MM-DD` 형식.
   - 내용은 "무엇을 왜 바꿨는지" 를 한 줄로 요약.

### 예시
```
| 2026-07-01 | hts_order_buy: 매수 재시도 로직 추가 (일시적 HTS 팝업 오류 대응) |
```
