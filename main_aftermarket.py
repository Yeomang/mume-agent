...
def run_aftermarket_job(is_test_mode: bool = False, manual: bool = False):
    """
    Aftermarket 추가 주문 전체 플로우.
    - is_test_mode: True면 테스트 모드
    - manual: 웹에서 수동 실행 여부
    """
    os.chdir(BASE_DIR)

    # ─────────────────────────────────────
    # 1) 환경변수로 사용자/계좌/사이클/테스트모드/조회기간 override
    # ─────────────────────────────────────
...
def main():
    ...
    register_job_pid("aftermarket")

    try:
        run_aftermarket_job(is_test_mode=False, manual=False)
    except Exception:
        logging.error("=== Exception in main_aftermarket.py::main() ===")
        logging.error(traceback.format_exc())
        raise
    finally:
        unregister_job_pid("aftermarket")
